...
#define DEVICE_NAME             ( "ssd1306_oled" )          // Device and Driver Name
#define SSD1306_ADDR            (       0x3C )              // SSD1306 OLED Slave Address
#define SSD1306_MAX_SEG         (        128 )              // Maximum segment
#define SSD1306_MAX_LINE        (          7 )              // Maximum line
#define SSD1306_FONT_SIZE       (          5 )              // Default font size
...
static void SSD1306_StartScrollHorizontal (bool is_left_scroll, uint8_t start_line_no,
                                           uint8_t end_line_no);
...
static uint8_t SSD1306_FontSize  = SSD1306_FONT_SIZE;
...
{
	if(is_left_scroll)
		SSD1306_Write(true, 0x27); // left horizontal scroll
	else
		SSD1306_Write(true, 0x26); // right horizontal scroll 
