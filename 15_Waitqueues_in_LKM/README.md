...
- **설명**:
  - 이 출력은 커널 모듈이 성공적으로 로드되었을 때의 상황을 보여줍니다.
  - `Device Number registered!`: 장치 번호가 정상적으로 등록되었음을 나타냅니다. 이 장치 번호는 모듈이 사용자 공간과 상호작용할 수 있는 인터페이스를 제공합니다.
  - `Thread 1 was created and is running now!` 및 `Thread 2 was created and is running now!`: 두 개의 커널 스레드가 각각 성공적으로 생성되어 실행 중임을 나타냅니다.
  - `Both threads are running now!`: 두 스레드가 모두 정상적으로 실행 중임을 알리는 메시지입니다.
...
- **설명**:
  - `write callback called`: 사용자 공간에서 커널 모듈로 데이터를 쓰는 작업이 발생했음을 나타냅니다. 이는 `my_write()` 함수에서 호출된 로그 메시지입니다.
  - `watch_var is now 22`: 사용자가 전달한 값이 `watch_var`에 22로 설정되었음을 나타냅니다.
  - `watch_var is now 22!`: `wq2`에서 대기 중이던 두 번째 스레드가 `watch_var == 22` 조건을 만족하여 깨어나면서 실행된 로그입니다.
  
  이 결과로 미루어 보아, 사용자 애플리케이션이 장치 파일을 통해 22라는 값을 `write` 했고, 이를 처리한 커널 모듈에서 대기 중이던 스레드가 조건을 만족하여 깨어났습니다.
...
- **설명**:
  - `write callback called`: 다시 사용자 공간에서 데이터를 쓰는 작업이 발생했음을 알립니다.
  - `watch_var is now 11`: 이번에는 사용자가 11이라는 값을 전달했음을 나타냅니다.
  - `watch_var is now 11!`: `wq1`에서 대기 중이던 첫 번째 스레드가 `watch_var == 11` 조건을 만족하여 깨어났습니다.

  여기서는 `watch_var`가 11로 설정되면서, 첫 번째 스레드가 대기 상태에서 벗어나 실행되었습니다. 
...
- **설명**:
  - 앞서 본 세 번째 사진과 유사한 결과로, `write callback called`는 쓰기 작업이 발생했음을 알립니다.
  - `watch_var is now 22`: 이번에도 사용자로부터 22가 전달되었고, `watch_var`가 22로 설정되었습니다.
  - `watch_var is now 22!`: `wq2`에서 대기 중이던 두 번째 스레드가 조건을 만족하여 깨어났습니다.

  이 결과는 다시 `watch_var == 22` 조건을 만족시켜 두 번째 스레드가 깨어나게 되는 과정을 보여줍니다.

### 종합 설명
- **대기 큐와 스레드**:
  - 이 모듈은 두 개의 대기 큐(`wq1`과 `wq2`)와 두 개의 커널 스레드를 사용합니다.
  - 스레드는 각각 `watch_var == 11` 또는 `watch_var == 22` 조건이 충족될 때까지 대기하며, 조건이 만족되면 실행됩니다.
  - 사용자 공간에서 장치 파일을 통해 `write` 작업을 수행하면 `watch_var`가 업데이트되며, 대기 중이던 스레드들이 깨워져 작업을 수행합니다.

- **결과 요약**:
  - 첫 번째 스레드는 `watch_var == 11` 조건이 만족되면 대기에서 풀려나고, 두 번째 스레드는 `watch_var == 22` 조건이 만족되면 대기에서 풀려납니다.
  - 사진에 나타난 로그는 각각의 스레드가 적절한 조건이 충족되었을 때 대기 큐에서 깨어나 실행된 상황을 보여줍니다.

이러한 로그들은 대기 큐를 사용한 커널 스레드의 동작을 보여주며, 커널 모듈이 올바르게 스레드를 관리하고 있음을 확인할 수 있습니다.
